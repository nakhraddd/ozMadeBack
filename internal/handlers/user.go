...
func UpdateProfile(c *gin.Context) {
	...
	if input.AddressLat != nil && input.AddressLng != nil {
		...
		c.JSON(http.StatusBadRequest, dto.ErrorResponse{Error: "address_lat and address_lng must be provided together"})
		return
	}
	if len(updates) > 0 {
...
func UpdateFCMToken(c *gin.Context) {
	userID := c.GetUint("userID")
	var input dto.UpdateFCMTokenInput
	if err := c.ShouldBindJSON(&input); err != nil {
		c.JSON(http.StatusBadRequest, dto.ErrorResponse{Error: err.Error()})
		return
	}
	...
	c.JSON(http.StatusOK, dto.MessageResponse{Message: "FCM token updated"})
}
...
func ToggleFavorite(c *gin.Context) {
	...
	if err := database.DB.Where("user_id = ? AND product_id = ?", userID, productID).First(&favorite).Error; err != nil {
		...
		c.JSON(http.StatusOK, dto.StatusResponse{Status: "added"})
	} else {
		database.DB.Delete(&favorite)
		c.JSON(http.StatusOK, dto.StatusResponse{Status: "removed"})
	}
}
